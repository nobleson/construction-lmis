...
<div class="row justify-content-center mt-3">
      <mdb-container>
         <b-link @click="$emit('changeComponent',{component: 'companiesView', id: null})"  href="#" class="card-link"><mdb-icon icon="arrow-left" size="lg" class="text-white" />  Back</b-link>
            <mdb-row class="mb-3">
            <mdb-col md="3" offsetMd="1" class="m-3">
              <mdb-view>
                <img  v-if="typeof companyData.logo == 'undefined'" src="~/assets/images/company.png" alt=""/>
                <img v-else :src="companyData.logo" alt=""/>
                <a><mdb-mask waves overlay="white-slight"/></a>
              </mdb-view>
            </mdb-col>
            <mdb-col md="8" class="text-md-left ml-3 mt-3">
              <h4 class="h4 mb-4">{{companyData.companyName}}</h4>
              <a href="#!" class="green-text">
                <h6 class="h6 pb-1">{{companyData.companyAcronym}}</h6>
              </a>
              <h5 v-if="companyData.companyType == 'Registered Building Contractor'" style="font-size: 18px"><mdb-badge pill color="primary"> {{companyData.companyType}}</mdb-badge></h5>
              <h5 v-else style="font-size: 18px"><mdb-badge pill color="danger"> {{companyData.companyType}}</mdb-badge></h5>
              <p class="font-weight-normal">{{companyData.companyAddress}}</p>
              <p class="font-weight-normal">From <a><strong>{{companyData.companyState}}</strong></a></p>
              <a style="font-size: 18px"><mdb-badge color="orange"><mdb-icon icon="phone" class="warning-color"/>{{companyData.companyTelephone}}</mdb-badge></a>
              <a  style="font-size: 18px"><mdb-badge pill color="teal"><mdb-icon icon="envelope" class="warning-color"/> {{companyData.companyEmail}}</mdb-badge></a>
            </mdb-col>
            </mdb-row>

            <mdb-card border="light" class="mb-3" >
            <mdb-card-header>Comapny Strength</mdb-card-header>
            <mdb-card-body>
               <!-- <mdb-row>
...
import {mdbFormInline, mdbContainer,mdbCard, mdbCol, mdbRow, mdbIcon, mdbTabs,
  ...
  export default {
    ...
    methods: {
     ...
     loadFacilities() {
          this.loadFacilitiesByProvider(this.companyData._id).then(e => this.getExistingFacilities(e)).catch(function(error){console.log(error.message)});
        },
